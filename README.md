...
1. Ensure a compatible .NET SDK is installed for the solution projects.
2. Clone or download the project.
3. Run `dotnet build DigimonTimeStrangerDB.sln`.
4. Run the console app from `DigimonDB` or launch the MAUI app from `DigimonDB.App`.

## Usage

- Use the menu or MAUI navigation to explore the available data views.
- Import data first to populate the sample Digimon, moves, items, and evolution data.
- Add or edit data through the available application flows as they are implemented.
...
- [DigimonDB.Core](DigimonDB.Core)
  ...
  - Contains `DigimonService`, `ItemService`, `ImportService`, and `DashboardService`
- [DigimonDB.App](DigimonDB.App)
  - .NET MAUI front end
  - UI pages, navigation, assets, and desktop/mobile app hosting

## Future Plans

- Implement full CRUD for Moves, Items, and Characters
- Expand the Python tooling that works with the shared SQLite database
- Add web scraping support for additional data sources
- Improve recovery and editing flows when unexpected data issues happen
